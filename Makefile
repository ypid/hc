...
.PHONY: check-pylint
check-pylint: hc/
	pylint "$<" --reports=n --rcfile .pylintrc --disable=missing-docstring
...
check-unit-tests-with-coverage:
	$(NOSETESTS) --with-coverage --cover-package hc --cover-branches --cover-erase --cover-html --cover-html-dir tests/coverage-report

.PHONY: check-integration-tests
check-integration-tests: tests
	hc -d -c ./tests/cache/ -F '2017-01' -T '2017-02' -f schulferien_html -t yaml -
	cd "$<" && bats .
...
.PHONY: check-fixmes
check-fixmes:
	ag '(:?[F]IXME|[T]ODO|[n]ottest)' --ignore 'hc/defaults.py' && exit 1 || :
...
release-versionbump: hc/_meta.py CHANGES.rst
	editor $?
